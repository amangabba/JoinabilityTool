...
def main():
    ...
    s_cols_by_prop, _ = f.get_properties_by_dataframe(source_original)
    t_cols_by_prop, _ = f.get_properties_by_dataframe(target_original)

    print("Removing columns by missing values...")
    source, s_removed_columns = f.remove_columns_by_missing_values(source_original, s_cols_by_prop['missing'], max_norm_missing_values)
    [print("Removed column", col_name, "from \"Table 1\", it has", miss_values, "missing values.",) for
     col_name, miss_values in s_removed_columns]
    target, t_removed_columns = f.remove_columns_by_missing_values(target_original, t_cols_by_prop['missing'], max_norm_missing_values)
    [print("Removed column", col_name, "from \"Table 2\", it has", miss_values, "missing values.", ) for
     col_name, miss_values in t_removed_columns]

    s_cols_by_prop = f.delete_properties_of_column(s_cols_by_prop, [x[0] for x in s_removed_columns])
    t_cols_by_prop = f.delete_properties_of_column(t_cols_by_prop, [x[0] for x in t_removed_columns])

    print("\nRemoving columns by average number of characters...")
    source, s_removed_columns = f.remove_columns_by_length(source, s_cols_by_prop['avg_chars'], max_avg_column_chars)
    [print("Removed column", col_name, "from \"Table 1\", it has", avg_chars, "average characters.", ) for
     col_name, avg_chars in s_removed_columns]
    target, t_removed_columns = f.remove_columns_by_length(target, t_cols_by_prop['avg_chars'], max_avg_column_chars)
    [print("Removed column", col_name, "from \"Table 2\", it has", avg_chars, "average characters.", ) for
     col_name, avg_chars in t_removed_columns]
    ...
    cc_sing_merge, cc_sing_disc = f.get_useful_columns(source, target, cc_sing, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)
    ...
    cc_multi_merge, cc_multi_dis = f.get_useful_columns(source, target, cc_multi, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)
    ...
    if not merge_columns:
        ...
        sc_sing_merge, sc_sing_disc = f.get_useful_columns(source, target, sc_sing, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)
        ...
        sc_multi_merge, sc_multi_disc = f.get_useful_columns(source, target, sc_multi, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)
        ...
        if not merge_columns:
            ...
            source, target, s_disc, t_disc = f.remove_columns_by_category_mismatch(source, target, s_cols_by_prop['category'], t_cols_by_prop['category'])
            s_cols_by_prop = f.delete_properties_of_column(s_cols_by_prop, s_disc)
            t_cols_by_prop = f.delete_properties_of_column(t_cols_by_prop, t_disc)
            print("Discarded columns for category mismatch: ", s_disc, t_disc)
            im_columns = []
            source_keys = f.get_unique_keys(source, s_cols_by_prop['category'], min_norm_uniq_rows)
            target_keys = f.get_unique_keys(target, t_cols_by_prop['category'], min_norm_uniq_rows)
            ...
            if (not source_keys) and (not target_keys):
                ...
                if source_keys:
                    [im_columns.append({"left": x, "right": y, "similarity_info": []}) for x, y in
                     f.find_foreign_keys(source_keys, t_cols_by_prop['category'])]
                if target_keys:
                    [im_columns.append({"left": x, "right": y, "similarity_info": []}) for x, y in
                     f.find_foreign_keys(target_keys, s_cols_by_prop['category'], keys_table="right")]
                ...
                for original, list_to_append in zip((merge_columns, discarded),
                                                    f.get_useful_columns(source, target, im_columns, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)):
            ...
            if not merge_columns:
                cm_pairs_found = []
                cm_pairs = []
                s_cols_by_cat = f.get_columns_grouped_by_category(s_cols_by_prop['category'])
                t_cols_by_cat = f.get_columns_grouped_by_category(t_cols_by_prop['category'])
                ...
                merge_columns, cm_discard = f.get_useful_columns(source, target, cm_pairs, s_cols_by_prop['category'], t_cols_by_prop['category'], min_join, eval_method, score_method)
    ...
    if merge_columns:
        ...
        [counter_cols["left"].append(col) for col in useful_cols["left"] if f.check_if_column_counter(source, col, s_cols_by_prop)]
        [counter_cols["right"].append(col) for col in useful_cols["right"] if f.check_if_column_counter(target, col, t_cols_by_prop)]
